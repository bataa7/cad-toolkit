...
def normalize_blocks(target_dir: str):
    ...
    for name, fps in name_to_fps.items():
        if len(fps) == 1:
            ...
            # We can't sort complex tuples easily if they contain mixed types, but let's try str(fp)
    ...
    for fp, names in fp_to_names.items():
        ...
        # If there's a tie or other criteria? 
        # For now, most frequent is good.
    ...
    for file_path, blocks in file_block_map.items():
        ...
        try:
            ...
            # Create a map of CurrentName -> CanonicalName for this file
            # And also handle local merges
            
            # We need to be careful about order.
            # 1. Rename blocks to temporary names if needed?
            # No, just iterate and plan.
            
            # Plan:
            # BlockName -> NewName
            # If multiple blocks map to same NewName, one survives, others merge.
            ...
            for name, fp in blocks:
                ...
                # First resolve the name conflict (v1, v2...)
                # Then map to canonical (aliasing)
                # But wait, fp_to_canonical uses the resolved name?
                # Actually fp_to_canonical maps FP -> FinalName directly.
            ...
            for final_name, current_names in target_groups.items():
                # current_names contains the names of blocks in this file that should all become final_name
                # If final_name is already one of them, great.
                # If not, we might need to rename one.
                
                unique_currents = list(set(current_names))
                
                # If multiple blocks in this file map to the same final_name
                ...
                if survivor_name != final_name:
                    # Check if final_name already exists (collision with a block we haven't processed yet?)
...
def _merge_block_references(doc, old_name, new_name):
    ...
    # Yes, iterate all block definitions and their entities
    for block in doc.blocks:
        if block.name == old_name: 
            continue # Don't update the block definition itself (it will be deleted)
        for entity in block:
            if entity.dxftype() == 'INSERT':
                if entity.dxf.name == old_name:
                    entity.dxf.name = new_name
